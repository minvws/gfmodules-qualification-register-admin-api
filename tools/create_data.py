...
from app.db.entities.application_version_qualification import ProtocolApplicationQualification
from app.db.entities.healthcare_provider import HealthcareProvider
from app.db.entities.healthcare_provider_application_version import HealthcareProviderApplicationVersion
from app.db.entities.healthcare_provider_qualification import HealthcareProviderQualification
...
parser.add_argument("-c", "--count", metavar="c", required=True, dest="count", help="A global number to give a indication of how many entities you want.", type=int)
...
def create_system_type(session: Session, name: str) -> SystemType:
    existing_system_type = session.scalars(select(SystemType).filter_by(name=name)).first()
    return existing_system_type if existing_system_type is not None else SystemType(name=name)
...
def create_applications(
    ...
    return [create_application(roles, system_types, protocols, healthcare_providers) for _ in range(count)]
...
def create_application(
    ...
    return Application(
        ...
        system_types=[ApplicationType(system_type=system_type) for system_type in random.sample(system_types, 2)],
        versions=[ApplicationVersion(
            version=version,
            qualified_protocol_versions=[
                ProtocolApplicationQualification(
                    protocol_version=random.choice(random.choice(protocols).versions),
            ...
            healthcare_providers=[
                ...
                for hpav
                in random.sample(healthcare_providers, min(random.randint(1,3), len(healthcare_providers)))]
...
def create_vendors(count: int, roles: List[Role], system_types: List[SystemType]) -> Sequence[Vendor]:
...
def random_protocols(protocols: List[Protocol]) -> List[ProtocolVersion]:
    ...
    for protocol in random.sample(protocols, randint(1, min(5, len(protocols)))):
        for pv in random.sample(protocol.versions, randint(1, min(5, len(protocol.versions)))):
            pvs.append(pv)
    return pvs

def create_healthcare_providers(count: int, protocols: List[Protocol]) -> List[HealthcareProvider]:
    company_name = fake.company()
